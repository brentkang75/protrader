...
# ================================
# FUSION ENGINE PRO — Confluence Weighted Professional
# ================================
class FusionEnginePro:
    """
    Fusion Engine Professional:
    Menggabungkan:
    - SMC/ICT PRO
    - Enhanced PRO
    - ICT Pure Analysis
    ...
    - News Sentiment
    - Fundamental Bias
    Hasil akhirnya = 1 sinyal final.
    """

    # Bobot profesional (dituning manual untuk hasil akurat)
    WEIGHTS = {
        "smc": 0.30,
        "technical": 0.15,
        "ml": 0.20,
    ...
    def compute_components(self, df, pair: str, tf: str):
        """
        Ambil score dari seluruh engine.
        """
        scores = {k: 0.0 for k in self.WEIGHTS.keys()}
        meta = {}

        # --- 1. PRO Signal (SMC/ICT+Technical+ML) ---
        try:
            pro = SMCICTProEngine.generate_ict_signal_pro(df, pair=pair, tf=tf)
            scores["smc"] = self.safe_conf(pro.get("confidence", 0))
            scores["technical"] = self.safe_conf(pro.get("technical_score", scores["smc"]))
            meta["pro"] = pro
        except Exception as e:
            meta["pro_error"] = str(e)

        # --- 2. Enhanced PRO (SMC+Fundamental+News+ML) ---
        try:
            enh = EnhancedSMCICTProEngine.generate_ict_signal_pro_with_fundamental(df, pair=pair, tf=tf)
            scores["fundamental"] = self.safe_conf(enh.get("fundamental_confidence", enh.get("confidence", 0)))
            scores["news"] = self.safe_conf(enh.get("news_adjustment", 0.5))
            scores["technical"] = max(scores["technical"], self.safe_conf(enh.get("technical_score", 0)))
            meta["enhanced"] = enh
        except Exception as e:
            meta["enhanced_error"] = str(e)

        # --- 3. ICT pure analysis ---
        try:
            ict = SMCICTProEngine.generate_ict_signal_pro(df, pair=pair, tf=tf, include_detailed_analysis=True)
            scores["ict"] = self.safe_conf(ict.get("confidence", 0))
            meta["ict"] = ict
        except Exception as e:
            meta["ict_error"] = str(e)

        # --- 4. ML MODEL ---
        try:
            if "MLModelManager" in globals():
                ml_prob = float(MLModelManager.predict_prob(df, pair=pair, tf=tf) or 0)
                scores["ml"] = self.safe_conf(ml_prob)
            else:
                scores["ml"] = (scores["smc"] + scores["technical"]) / 2
        except:
            scores["ml"] = (scores["smc"] + scores["technical"]) / 2

        return scores, meta

    def fused_confidence(self, scores):
        """ Weighted confluence """
        total = 0
    ...
    def final_signal(self, df, pair, tf):
        """
        Hasil akhir satu sinyal paling akurat.
        """
        scores, meta = self.compute_components(df, pair, tf)
        fused = self.fused_confidence(scores)

        # --- Tentukan arah sinyal ---
        if fused >= 0.80:
            direction = "STRONG LONG" if scores["smc"] >= scores["ict"] else "STRONG SHORT"
        elif fused >= 0.60:
            direction = "LONG" if scores["smc"] >= scores["ict"] else "SHORT"
        else:
            direction = "WAIT"

        # --- Tentukan Entry/TP/SL ---
        try:
            base = meta["enhanced"]
        except:
            try:
        ...
        entry = float(base.get("entry", close)) if base else close
        sl = float(base.get("sl", close * 0.99)) if base else close * 0.99
        tp1 = float(base.get("tp1", close * 1.01)) if base else close * 1.01
        tp2 = float(base.get("tp2", tp1 * 1.01)) if base else close * 1.02
...
# ============= ENDPOINT: /fusion_pro_signal ================
@app.get("/fusion_pro_signal")
def fusion_pro_endpoint(pair: str, tf_entry: str = "15m", limit: int = 300):
    try:
        df = DataFetcher.fetch_ohlc_any(pair, tf_entry, limit)
        engine = FusionEnginePro()
        res = engine.final_signal(df, pair, tf_entry)
        return respond(res)
    except Exception as e:
        return respond({"error": str(e)}, status_code=500)
